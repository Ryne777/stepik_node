...
function parseTodo(requestBody) {
  const todo = {
    ...
    */
    ...
    completedAt: requestBody.completed ? new Date(Date.now()) : null
...
router.get('/', totalMiddleware, async (ctx, next) => {
  const { contentType, completed, ...queryFilter } = ctx.query
  if (completed != undefined) {
    queryFilter['completed'] = parseFilterValue(completed)
  }
  const filter = {
    /*
...
router.post('/', koaBody({ multipart: true }), totalMiddleware, async (ctx, next) => {
  ...
  /*
    ...
    todo.email = ...
  */
  todo.email = ctx.state.user.email
