...
async function deleteTodo(query) {
  ...
  /*
    ...
    */
  id = _mapObjectId(query)
  res = await col.findOne(id)
  if (res==null) {
    return false
  }

  await col.deleteOne(id)
  result = await col.updateOne(id, { $set: {} })
...
function getTodo(query) {
  ...
  */
  var todo = col.findOne(_mapObjectId(query))
  return todo
}
...
async function updateTodo(query, data) {
  ...
  res = await col.updateOne(_mapObjectId(query), update)
